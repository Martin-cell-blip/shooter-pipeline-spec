...
| 温斯顿 | 生命值 | — | 275 开放队列；425 职责队列；325 为 6v6；护甲 200 | 现场核验确认原抓取遗漏队列标签；职责队列与 API 的 625 合计相符，RESOLVED，快照限定职责队列 |

温斯顿的核验过程、屏障冷却起算与适用限制见 [来源核验记录](WINSTON_SOURCE_REVIEW.md)。这是来源问题的关闭，不是整个补丁的人工最终签字。
