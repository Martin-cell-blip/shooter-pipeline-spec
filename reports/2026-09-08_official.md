...
**结论：READY_FOR_HUMAN_REVIEW**（exit 0）— FAIL 0 / REVIEW 5 / NOT_RUN 10 / PASS 42
...
| P0_provenance_status | winston | open source conflict on hitpoints: {'overfast_api': {'health': 425, 'armor': 200, 'total': 625}, 'wiki_fandom': '信息框首个数字 Health 275（页面同时列多个模式/版本的取值，本次抓取只取到首项）'} — 两源差距过大，不像更新滞后，更像口径不同（5v5 vs 6v6，或含/不含护甲）；本仓库不猜，留空待人工核对后填入并关闭 |
...
| F3_hitpoints_sum | winston | hitpoints is null (open conflict or missing) — no numeric judgement |
| R1_full_cycle_dps_delta | winston | tesla_cannon_alt: ammo or reload_s missing (full-cycle DPS needs both) |
| R10_uptime_ratio | winston | barrier_projector[v5]: cooldown_starts unknown — duration/cooldown must not be presented as coverage |
