...
退出码：`0` 无 FAIL（允许 REVIEW / 缺输入的 NOT_RUN）；`1` 有 FAIL；`2` 加载失败或检查抛异常。金标样本当前结果：0 FAIL，REVIEW 与 NOT_RUN 见 `reports/2026-09-08_official.md`——其中 NOT_RUN 全部来自故意留空的输入（温斯顿生命值冲突、冷却起算方式未知、蓄力条件缺失），不是缺陷。
